...
// ADNS stuff
byte initComplete=0; // initialization status
volatile int xydat[2]; // stores X and Y movement measurements
//volatile byte burstData[6]; // stores motion burst data
const int ncs = 10; // SS - signal select, HIGH unless a transaction is happening

// store motion burst data
enum MotionBurst {
...
// RN-42 stuff
const int bluetoothTx = 4;  // TX-O pin of bluetooth mate, Arduino D4
const int bluetoothRx = 3;  // RX-I pin of bluetooth mate, Arduino D3

SoftwareSerial bluetooth(bluetoothTx, bluetoothRx);

// multimeter sensors
const int activeSwitch = 5; // send HID report if HIGH
const int dPin = 7; // connectivity digital input connected to pin 5
const int aPin = A0; // voltage analog input connected to A0
volatile int voltage; // stores measured voltage
volatile int pVoltage = 0; // stores previous voltage measurement
volatile int Vdel; // change in voltage
int mvtScale = 4; // divide X and Y readings by this number?
...
byte adns_read_reg(byte reg_addr){
  ...
  delay(1); // tSCLK-NCS for read operation is 120ns
  adns_com_end();
  delay(19); //  tSRW/tSRR (=20us) minus tSCLK-NCS
...
void adns_write_reg(byte reg_addr, byte data){
  ...
  delay(20); // tSCLK-NCS for write operation
  adns_com_end();
  delay(100); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound 
}
...
// Following few methods from 
// https://github.com/rpriyadarshi/ADNS9800_LCD/blob/master/ADNS9800.h
void read_motion_burst_data() {
  adns_com_begin();
...
uint16_t join_byte(byte l, byte h){
  uint16_t b = l;
  b |= (h << 8);
  return b;
}
...
int16_t convert_twos_compliment(uint16_t b){
  int16_t val = b;
  //Convert from 2's complement
  if (b & 0x8000) {
    val = -1 * ((b ^ 0xffff) + 1);
...
void copy_data() {
  _squal = _data[SQUAL];
  _ux = convert_twos_compliment(_data[Delta_X_L]);
  _uy = convert_twos_compliment(_data[Delta_Y_L]);
  _ux_dist += _ux;
  _uy_dist += _uy;
}

void update_motion_burst_data() {
  ...
  if (!_fault && _mot) {
    copy_data();
    _moved = 1;
    //Serial.println("got motion burst");
  }
  adns_com_end();
}
...
void setup() {
  // Bluetooth serial set up
  Serial.begin(9600);  // Begin the serial monitor at 9600bps
  pinMode(dPin, INPUT_PULLUP);
  pinMode(aPin, INPUT);
  pinMode(activeSwitch, INPUT_PULLUP);
...
void loop() {
  // is multimeter active?
  bool active = digitalRead(activeSwitch);
  
  if (active) {
    // get multimeter measurements
    bool ifConductive = digitalRead(dPin);
    ...
    if (voltage > 0) {
      ...
      //Serial.println("recording");
    	...
    	bluetooth.write((byte)0); // no clicks
    }
  
    // bytes #3, 4 - XY movement
    bluetooth.write(Xdel);
    bluetooth.write(Ydel);
    //Serial.println("movement sent");
    
    // byte #5 - wheel movement = voltage measurement
    bluetooth.write(Vdel);
    Serial.println(Vdel); 
  }

  // Still able to use this as a Bluetooth pipeline/enter configuration mode
  // check Bluetooth for any incoming data
